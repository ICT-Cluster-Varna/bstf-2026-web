/* OrbitControls r160 — global build (no ES module imports) */
(function () {
  'use strict';
  ...
  class OrbitControls extends EventDispatcher {
	...
	constructor( object, domElement ) {
		...
		function updateZoomParameters( x, y ) {
			...
			const dx = x - rect.left, dy = y - rect.top, w = rect.width, h = rect.height;
			mouse.x = ( dx / w ) * 2 - 1;
			mouse.y = - ( dy / h ) * 2 + 1;
		...
		function handleTouchStartDolly( event ) {
			...
			const dx = event.pageX-p.x, dy = event.pageY-p.y;
		...
		function onPointerMove( event ) {
			if ( scope.enabled === false ) return;
			if ( event.pointerType === 'touch' ) onTouchMove( event ); else onMouseMove( event );
		}
		...
		function onMouseDown( event ) {
			let mouseAction;
			switch ( event.button ) { case 0: mouseAction = scope.mouseButtons.LEFT; break; case 1: mouseAction = scope.mouseButtons.MIDDLE; break; case 2: mouseAction = scope.mouseButtons.RIGHT; break; default: mouseAction = -1; }
			switch ( mouseAction ) {
				case MOUSE.DOLLY:
		...
		function removePointer( event ) {
			...
			for ( let i = 0; i < pointers.length; i++ ) {
				if ( pointers[i] == event.pointerId ) { pointers.splice( i, 1 ); return; }
			}
		}
		function trackPointer( event ) {
			let position = pointerPositions[ event.pointerId ];
			if ( position === undefined ) { position = new Vector2(); pointerPositions[ event.pointerId ] = position; }
